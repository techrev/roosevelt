...
module.exports = function (app, callback) {
  ...
  logger.verbose(`called htmlValidator, line 22`)
  ...
  if (params.htmlValidator.enable) {
    ...
    logger.verbose('calling callValidator')
    callValidator()
  } else {
  ...
  function callValidator () {
    logger.verbose(`Called callValidator htmlValidator line 50, creating Promise`)
    return new Promise((resolve, reject) => {
      // see if there's one already running
      logger.verbose('checking for other running validators htmlValidator line 53')
      http.get(validatorOptions, (res) => {
        logger.verbose(`htmlValidator line 55: response from http get request received (sent to ${validatorOptions.url}: ${validatorOptions.port})`)
        const { statusCode } = res
        logger.verbose(`htmlValidator line 57: status code is ${statusCode}`)
        let error
        let rawData = ''
        if (statusCode !== 200) {
          logger.verbose(`htmlValidator line 61: oops! status code isn't 200...creating Error`)
          error = new Error(`Request Failed.\nStatus Code: ${statusCode}`)
        }
        if (error) {
          logger.verbose(`htmlValidator line 65: callValidator get request the error message is ${error.message}`)
        ...
        res.on('end', () => {
          logger.verbose(`All done receiving data. rawData is ${rawData}`)
        ...
        logger.verbose(`htmlValidator line 93: Oh dear. The call to get errored with ${err}. Spawning a new validator`)
        // spawn a new one
        javaCheck(spawnNewValidator)
      })

      function spawnNewValidator () {
        logger.verbose(`htmlValidator line 99: spawning a new validator since java is there`)
      ...
      function javaCheck (cb) {
        logger.verbose(`in javaCheck, htmlValidator line 125`)
        javaDetectProcess = spawn('java', ['-version'])
        javaDetectProcess.on('close', (data) => {
          logger.verbose(`htmlValidator line 129: javaDetectProcess closed with message: ${data}`)
        })
        javaDetectProcess.on('disconnect', (data) => {
          logger.verbose(`htmlValidator line 132: javaDetectProcess disconnected with message: ${data}`)
        })
        javaDetectProcess.on('exit', (data) => {
          logger.verbose(`htmlValidator line 135: javaDetectProcess exited with message: ${data}`)
        })
        javaDetectProcess.on('message', (data) => {
          logger.verbose(`htmlValidator line 138: javaDetectProcess sent message: ${data}`)
        })
        ...
        javaDetectProcess.stdout.on('data', (data) => {
          logger.verbose(`htmlValidator line 144: data from javaCheck stdout is ${data}`)
        })
        javaDetectProcess.stderr.on('data', (data) => {
          logger.verbose(`htmlValidator line 147: data from javaCheck stderr is ${data}`)
          if (data.includes('java version')) {
            logger.verbose(`htmlValidator line 149: java found. Calling callback (spawnNewValidator)`)
            cb()
          }
        })
      }
  ...
  function autoKillerStart (cb) {
    ...
    if (params.htmlValidator.separateProcess.enable && params.htmlValidator.separateProcess.autoKiller) {
      ...
      logger.verbose(`In htmlValidtor line 303, PIDPath is ${PIDPath}`)
