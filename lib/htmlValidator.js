...
module.exports = function (app, callback) {
  ...
  function autoKillerStart (cb) {
    ...
    if (params.htmlValidator.separateProcess.enable && params.htmlValidator.separateProcess.autoKiller) {
      ...
      logger.verbose(`in htmlValidator line 415, PIDPath is ${PIDPath}`)
      let test = fs.existsSync(PIDPath)
      logger.verbose(`in htmlValidator line 417, test for existence of ${PIDPath} is ${test}`)
      if (test === true) {
        ...
        logger.verbose(`in htmlValidator line 422, PID is ${PID}`)
        require('process-exists')(PID).then(exists => {
          logger.verbose(`in htmlValidator line 424, ${PID} exists? ${exists}`)
        })
        fkill(PID, { force: true, tree: false }).then((msg) => {
          logger.verbose(`in htmlValidator line 427, fkill promise resolved`)
