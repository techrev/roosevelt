...
module.exports = function (app, callback) {
  ...
  function callValidator () {
    logger.verbose('called callValidator, creating Promise')
    return new Promise((resolve, reject) => {
      // see if there's one already running
      logger.verbose('checking for other running validators')
      http.get(validatorOptions, (res) => {
        logger.verbose(`response from http get request received (sent to ${validatorOptions.url})`)
        const { statusCode } = res
        logger.verbose(`status code is ${statusCode}`)
        let error
        let rawData = ''
        if (statusCode !== 200) {
          logger.verbose(`oops! status code isn't 200...creating Error`)
          error = new Error(`Request Failed.\nStatus Code: ${statusCode}`)
        }
        if (error) {
          logger.verbose(`the error message is ${error.message}`)
        ...
        logger.verbose(`Oh dear. The call to get errored with ${err}. Spawning a new validator`)
        // spawn a new one
      ...
      function spawnNewValidator () {
        ...
        logger.verbose(`Spawning a new validator from ${stack}.`)
