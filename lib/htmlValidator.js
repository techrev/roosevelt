...
module.exports = function (app, callback) {
  ...
  function callValidator () {
    return new Promise((resolve, reject) => {
      ...
      logger.verbose(`in htmlValidator line 50, validatorOptions are:`)
      Object.keys(validatorOptions).forEach((key) => {
        logger.verbose(`${key}: ${validatorOptions[key]}`)
      })
      http.get(validatorOptions, (res) => {
        const { statusCode } = res
        logger.verbose(`in htmlValidator line 56, res status code is ${statusCode}`)
        let error
        let rawData = ''
        if (statusCode !== 200) {
        ...
        res.on('end', () => {
          logger.verbose(`in htmlValidator line 75, completed raw data is ${rawData}`)
        ...
        logger.verbose(`in htmlValidator line 89, received error message from get request: ${err}`)
        // spawn a new one
        javaCheck(spawnNewValidator)
      })

      function spawnNewValidator () {
        logger.verbose(`in htmlValidator line 95, about to spawn the html validator`)
        // let start = Date.now()
        ...
        validatorProcess.stdout.on('data', (data) => {
          logger.verbose(`htmlValidator line 106 data from validatorProcess stdout is ${data}`)
        ...
        validatorProcess.stderr.on('data', (data) => {
          logger.verbose(`htmlValidator line 229 data from validatorProcess stderr is ${data}`)
        ...
        validatorProcess.on('error', (err) => {
          logger.verbose(`htmlValidator line 243 error thrown in validatorProcess. Error is ${err.message}`)
        })
        ...
        validatorProcess.on('close', (code, signal) => {
          logger.verbose(`htmlValidator line 255 received close in validatorProcess. The code is ${code}. The signal is ${signal}`)
        })

        validatorProcess.on('exit', (code, signal) => {
          logger.verbose(`htmlValidator line 259 received exit in validatorProcess. The code is ${code}. The signal is ${signal}`)
        })
      ...
      function javaCheck (cb) {
        ...
        javaDetectProcess.stderr.on('data', (data) => {
          if (data.includes('java version')) {
            logger.verbose(`in htmlValidator line 290, java found, about to spawn validator`)
            cb()
          }
        })
      }
    }).then(result => {
      logger.verbose(`in htmlValidator line 294 result from callValidator promise is:`)
  ...
  function applyValidatorMiddleware () {
    ...
    app.use(tamper((req, res) => {
      ...
      function parseValidatorMessage (data) {
        ...
        validationMessage += '\n\n'
        logger.verbose(`in htmlValidator line 362, validationMessage is ${validationMessage}`)
