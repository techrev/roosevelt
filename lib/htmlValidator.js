...
module.exports = function (app, callback) {
  ...
  function callValidator () {
    ...
    return new Promise((resolve, reject) => {
      ...
      function javaCheck (cb) {
        logger.verbose(`in javaCheck, htmlValidator line 126`)
        ...
        javaDetectProcess.stdout.on('data', (data) => {
          logger.verbose(`htmlValidator line 133: data from javaCheck stdout is ${data}`)
        })
        javaDetectProcess.stderr.on('data', (data) => {
          logger.verbose(`htmlValidator line 136: data from javaCheck stderr is ${data}`)
          if (data.includes('java version')) {
            logger.verbose(`htmlValidator line 138: java found. Calling callback (spawnNewValidator)`)
            cb()
          }
        })
      }
    }).then(result => {
      logger.verbose(`htmlValidator line 144: Promise resolved? result is ${result}`)
      clearTimeout(validatorTimeout)
      autoKillerStart(callback)
    }).catch(error => {
      logger.verbose(`htmlValidator line 148: Danger Will Robinson! Error spawning java! Error is ${error.message}`)
      clearTimeout(validatorTimeout)
  ...
  function autoKillerStart (cb) {
    ...
    if (params.htmlValidator.separateProcess.enable && params.htmlValidator.separateProcess.autoKiller) {
      ...
      logger.verbose(`In htmlValidtor line 294, PIDPath is ${PIDPath}`)
