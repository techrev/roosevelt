...
function startTimeout () {
  timeoutHolder = setTimeout(() => {
    ...
    logger.verbose(`About to send get request with options:`)
    Object.keys(options).forEach((key) => {
      if (typeof options[key] === 'object') {
        logger.verbose(`${key}:`)
        Object.keys(options[key]).forEach((optionKey) => {
          logger.verbose(`${optionKey}: ${options[key][optionKey]}`)
        })
      } else {
        logger.verbose(`${key}: ${options[key]}`)
      }
    })
    http.get(options, function (res) {
      ...
      res.on('data', (data) => {
        logger.verbose(`Received response from http get request in autoKillValidator line 32: ${data}`)
      })
